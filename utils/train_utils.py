...
def load_checkpoint(
    ...
    optimizer: Adam,
    scheduler: LambdaLR,
) -> Tuple[nn.Module, Adam, Union[LambdaLR, None], int, Union[Dict[str, float], None], Dict[str, List[float]], Dict[str, float]]:
    ...
    if os.path.exists(ckpt_path):
        ...
        start_epoch = 1
        loss_info, hist_scores = None, {"mae": [], "rmse": [], "mrae": [], "rmrse": []}  # mrae: mean relative absolute error; rmrse: root mean relative squared error
        best_scores = {k: torch.inf for k in hist_scores.keys()}
        print(f"Checkpoint not found at {ckpt_path}.")

    return model, optimizer, scheduler, start_epoch, loss_info, hist_scores, best_scores
